...
const studies = [
  { key: "hemogram", label: "Hemograma", reference: 50 },
  { key: "hemogramResonance", label: "Hemograma + resonancia", reference: 200 },
  { key: "resonance", label: "Resonancia", reference: 120 },
  { key: "urine", label: "Análisis de orina", reference: 100 }
] as const;
...
export function PaidStudyOrderDialog({
  ...
  const [selected, setSelected] = useState<Record<StudyKey, boolean>>({
    hemogram: false,
    hemogramResonance: false,
    resonance: false,
    urine: false
  });
  const [prices, setPrices] = useState<Record<StudyKey, string>>({
    hemogram: "50.00",
    hemogramResonance: "200.00",
    resonance: "120.00",
    urine: "100.00"
  });
  ...
  const subtotal = useMemo(
    () =>
      studies.reduce(
        (sum, study) => sum + (selected[study.key] ? Number(prices[study.key]) || 0 : 0),
        0
      ),
    [prices, selected]
  );
  ...
  const hasSelection = studies.some((study) => selected[study.key]);
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[calc(100dvh-2rem)] w-[min(94vw,680px)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-[9px] border border-border bg-surface p-5 shadow-2xl sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <Dialog.Description className="mt-1 text-sm text-muted">
                Selecciona los análisis. La ficha pasará primero a Administración para el cobro.
              </Dialog.Description>
            </div>
          ...
          <form action={action} className="mt-5 grid gap-4">
            ...
            <div className="grid gap-2">
              {studies.map((study) => {
                const enabled = selected[study.key];
                return (
                  <div
                    key={study.key}
                  ...
                  >
                    <label className="flex min-h-11 items-center gap-3 text-sm font-medium text-text">
                      <input
                        ...
                        onChange={(event) =>
                          setSelected((current) => ({
                            ...current,
                            [study.key]: event.target.checked
                          }))
                        }
                      />
                      <span>
                        ...
                        <span className="mt-0.5 block text-xs font-normal text-muted">
                          Referencia: {study.reference.toFixed(2)} Bs
                        </span>
                      </span>
                    </label>
                    <Field label="Precio Bs">
                      <input
                        ...
                        name={`${study.key}Price`}
                        inputMode="decimal"
                        value={prices[study.key]}
                        disabled={!enabled}
                        required={enabled}
                        onChange={(event) =>
                          setPrices((current) => ({
                            ...current,
                            [study.key]: event.target.value
                          }))
                        }
                      />
                    </Field>
